...
/* ====================== KEGIATAN ====================== */
const activityForm = useForm({ kode_kegiatan: '', nama_kegiatan: '' })
...
const submitActivity = () => {
    activityForm.post(`/program/${props.program.id}/activity`, {
        ...
        onSuccess: () => {
            activityForm.reset()
            $inertia.visit(window.location.href, { preserveScroll: true })
        }
    })
}
...
const deleteActivity = (id) => {
    if (confirm('Yakin ingin menghapus kegiatan ini?')) {
        useForm().delete(`/activity/${id}`, {
            preserveScroll: true,
            onSuccess: () => $inertia.visit(window.location.href, { preserveScroll: true })
        })
    }
}
...
const subForm = useForm({
    kode_sub_kegiatan: '', nama_sub_kegiatan: '', pagu_anggaran: '', sumber_dana: '',
...
<template>
    ...
    <form v-if="program.status === 'draft'" @submit.prevent="submitActivity" class="mb-6">
        <input v-model="activityForm.kode_kegiatan" type="text" placeholder="Kode Kegiatan" class="border p-2 mr-2" />
        <input v-model="activityForm.nama_kegiatan" type="text" placeholder="Nama Kegiatan" class="border p-2 mr-2" />
    ...
    <div v-for="activity in program.activities" :key="activity.id" class="border p-4 mb-6">
        ...
        <div v-if="editingActivityId !== activity.id">
            <h3 class="font-semibold mb-2">{{ activity.nama_kegiatan }}</h3>
            <div class="text-sm text-green-400 mb-2">
                Total Pagu: Rp {{ activity.total_pagu?.toLocaleString('id-ID') || 0 }}
            </div>
            <div class="space-x-2 mb-4">
                <button v-if="program.status === 'draft'" @click="startEditActivity(activity)" class="bg-yellow-500 px-3 py-1 text-black">Edit</button>
                <button v-if="program.status === 'draft'" @click="deleteActivity(activity.id)" class="bg-red-600 px-3 py-1 text-white">Hapus</button>
            </div>
        </div>
        ...
        <!-- FORM SUB KEGIATAN -->
        <form v-if="program.status === 'draft'" @submit.prevent="submitSub(activity.id)" class="space-y-2 mb-4">
            <input v-model="subForm.kode_sub_kegiatan" type="text" placeholder="Kode Sub Kegiatan" class="border p-2 w-full" />
            <input v-model="subForm.nama_sub_kegiatan" type="text" placeholder="Nama Sub Kegiatan" class="border p-2 w-full" />
            <input v-model="subForm.pagu_anggaran" type="number" placeholder="Pagu Anggaran" class="border p-2 w-full" />
            <input v-model="subForm.indikator" type="text" placeholder="Indikator" class="border p-2 w-full" />
            <input v-model="subForm.target" type="text" placeholder="Target" class="border p-2 w-full" />
            <input v-model="subForm.prioritas_provinsi" type="text" placeholder="Prioritas Provinsi" class="border p-2 w-full" />
            <input v-model="subForm.prioritas_kabupaten" type="text" placeholder="Prioritas Kabupaten" class="border p-2 w-full" />
            <input v-model="subForm.bidang_urusan" type="text" placeholder="Bidang Urusan" class="border p-2 w-full" />
            <input v-model="subForm.n1" type="number" placeholder="N-1" readonly class="bg-gray-25" />
            <input v-model="subForm.n2" type="number" placeholder="N-2" readonly class="bg-gray-25" />
            <button class="bg-green-500 text-white px-4 py-2">Tambah Sub</button>
        </form>
            ...
            <div v-for="sub in activity.sub_activities" :key="sub.id" class="border p-3 mb-3 bg-gray-800">
                ...
                <div v-if="editingId !== sub.id">
                    ...
                    <div class="mt-2 space-x-2">
                        <button v-if="program.status === 'draft'" @click="startEdit(sub)" class="bg-yellow-500 px-3 py-1 text-black">Edit</button>
                        <button v-if="program.status === 'draft'" @click="deleteSub(sub.id)" class="bg-red-600 px-3 py-1 text-white">Hapus</button>
                    </div>
                </div>
